...
public class LinkTag extends WikiLinkTag implements ParamHandler, BodyTag {
        ...
        throws ProviderException
    {
        ...
        if( m_templatefile != null )
        {
            ...
            String path = "templates/"+template+"/"+m_templatefile;
            try {
                path = new URI(null, null, path, null).normalize().toString();
            } catch (URISyntaxException e) {
                log.error("Incorrect path was provided: " + path);
            }
            url = engine.getURL( WikiContext.NONE, path, params, false );
        }
        else if( m_jsp != null )
        {
            String params = addParamsForRecipient( null, m_containedParams );
