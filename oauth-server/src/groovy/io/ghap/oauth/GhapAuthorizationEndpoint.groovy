...
class GhapAuthorizationEndpoint extends AuthorizationEndpoint {
    ...
    public ModelAndView authorize(Map<String, Object> model, @RequestParam Map<String, String> parameters,
        ...
        try {
            ...
            logger.debug("Set redirect URI to: " + resolvedRedirect)
    ...
    private ModelAndView getImplicitGrantResponse(AuthorizationRequest authorizationRequest) {
        try {
            ...
            logger.error("Error during rendering redirect model and view.", e)
    ...
    private OAuth2AccessToken getAccessTokenForImplicitGrant(TokenRequest tokenRequest,
        ...
        synchronized (this.implicitLock) {
            def granter = getTokenGranter()
            logger.info("granter class " + granter.class.name)
            accessToken = granter.grant("implicit",
    ...
    private String appendAccessToken(AuthorizationRequest authorizationRequest, OAuth2AccessToken accessToken) {
        ...
        logger.info("start populate additional parameter for " + accessToken.getValue())
        for (String key : additionalInformation.keySet()) {
            logger.info("additional param key = " + key)
            if ("nonce".equals(key) || "subject".equals(key)) {
                continue;
            }
            Object value = additionalInformation.get(key);
            if (value != null) {
                logger.info("additional param key = " + key + " value = " + value)
                String prefix = "id_token".equals(key) ? "" : "extra_"
                keys.put(prefix + key, key);
                vars.put(prefix + key, value);
            }
        }
        // Do not include the refresh token (even if there is one)
        String redirectUri = authorizationRequest.getRedirectUri()
        redirectUri = append(redirectUri, vars, keys, !redirectUri.contains("tokenViaQueryString"))
        logger.debug("Redirect URI with access token: " + redirectUri)
        return redirectUri
    }
