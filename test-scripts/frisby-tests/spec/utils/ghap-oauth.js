...
GhapOAuth.prototype.login = function (username, password) {
	...
	var start = new Date();
	var self = this;
	console.log("OAuth authorization started for user '%s'.", username);
		...
		request({
			...
			request({
				...
				if (matches != null) {
					...
					if (self.doAfter !== null) self.doAfter();
				} else
...
GhapOAuth.prototype.then = function(do_after) {
	this.doAfter = do_after;
	if (this.access_token !== null)	this.doAfter();
};
