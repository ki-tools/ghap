...
if (args.length < 1){
	console.log("Use: node %s [environment] path", process.argv[1]);
	process.exit(1);
}

var delete_project_at_final = false;

var arg_num = 0;
while (args[arg_num].substr(0,2) === '--') {
	var option_name = args[arg_num].substr(2);
	if (option_name === 'delete') {
		delete_project_at_final = true;
		console.log('Projects created by script will be deleted at final of script execution.')
	} else {
...
function final(){
	...
	if (delete_project_at_final === true) {
...
function pushToStash(grant_path, prj_key, grant_name, callback) {
	...
	runCommand('git remote add origin '+ cfg.stashRepoBase + '/' + prj_key.toLowerCase()
	   +'/'+ grant_name.toLowerCase() +'.git');
