...
function createUsers(prev_results){
	...
	testUsers.forEach(function (ums_user) {
		createCalls.push(
			function (next) {
				umsRequests.createUser(authHeader, ums_user, next);
			}
		);
	});
...
function deleteSomeUsers(prev_results) {
	...
	ctrlFlow.series([
		function(next) {umsRequests.deleteUser(authHeader, user_4, next); }
...
function deleteAllUsers(prev_results) {
	...
	testUsers.forEach(function(user){
		if (user !== 'undefined')
			deleteCalls.push( function (next) {umsRequests.deleteUser(authHeader, user, next);} );
	});
	ctrlFlow.series(deleteCalls, deleteGroups);
}
